...
- `launchApp` currently always passes `-D` (debug) so the first operation after launch is a JDWP attach. The `resumeProcess: true` default for `launch` requests exists specifically to release that `-D` pause when `mode === "native"` (the java debugger would have resumed it otherwise).
